...
**What was built from it, and re-checked 22 September 2026.** The agreement
check answers `ask` or `deny` or stays silent — never `allow`, which would
override the developer's own rules, and never `defer`, which an interactive
session ignores and `-p` treats as a pause. The same reference now also says a
hook that exits non-zero without JSON, or cannot start at all, lets the call
through; only exit 2 or a JSON `deny` blocks. So the check denies at its own
...
*25 September 2026.* These are six public claims about Codex that an adapter would rely on, checked against one machine's `~/.codex`. The tool used is the desktop app (`originator: codex_work_desktop`, `source: vscode`), not the standalone CLI. Two rollouts were read in full: `~/.codex/sessions/2026/09/24/rollout-2026-09-24T14-03-51-01a0d496-8b44-7e71-b580-7fd481c80b11.jsonl` (A) and `~/.codex/sessions/2026/09/22/rollout-2026-09-22T12-49-25-01a0ca05-af12-7652-af8d-2e7ba556107d.jsonl` (B). The two newest files, from 25 September, each hold one `task_started` (turn id `external-import-turn-1`), no `turn_context` and no tool calls, so they could not answer questions 2 or 4. The adapter counts turns by `task_started`, but excludes imported history by its `external-import-` turn-id prefix, counting it as `importedTurnsSkipped` rather than as a turn, so it parses no turn from either file and one skipped import from each; if Codex renames the prefix, the import test in `test/codex.test.ts` is the tripwire. Only record types, keys and counts were read. No prompt or message text is quoted here.
