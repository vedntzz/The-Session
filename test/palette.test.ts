...
const clean = session({
  ...
  drift: [],
  outcome: "abandoned",
  // Marked by hand, so `week` still prints the word and the ink that goes
  // with it: a computed `abandoned` reads `open` there — see `reportedOutcome`.
  observations: [
    {
      outcome: "abandoned",
      observedAt: at(9, 30, 16),
      commit: "def5678",
      branch: "origin/main",
      source: "manual",
    },
...
function everything(palette: Palette): string[] {
  return [
    ...formatSession(drifted, palette, { ...priced, tokens: true, checked: "2026-08-23" }),
    ...
    // With a date on the prices, so the line under the money is rendered by
    // both palettes and pinned like the rest.
    ...formatWeek([drifted], 7, palette, { client: "Acme" }, { ...priced, checked: "2026-08-23" }),
...
const GOLDEN: readonly string[] = [
  ...
  "",
  "  The last 7 days",
  "  one block per intent source, never pooled and never totalled: declared, primed and captured are different evidence, and one figure over all three would move whenever their mix moved",
  "",
  "  declared · 3 sessions · 1 landed on the default branch · 1 marked abandoned · 1 still open",
  "  1 path outside what was declared, in 1 of 3 sessions that declared one",
  "                                             outside  turns     tokens  no edits     cost",
  "  add rate limiting to /orders",
  "    11111111  01-15 14:02  api    merged           1      7  1,105,600         2    $3.69",
  "  extract the store",
  "    11111111  01-16 08:31  other  abandoned        0      3      1,100         0    $0.02",
  "  try the new model",
  "    11111111  01-17 10:00  other  open             0      1        500         1  unknown",
  "",
  "  primed · no sessions",
  "",
  ...
  "  }",
  "",
  "  The last 7 days, only client Acme",
  "  one block per intent source, never pooled and never totalled: declared, primed and captured are different evidence, and one figure over all three would move whenever their mix moved",
  "",
  "  declared · 1 session · 1 landed on the default branch",
  "  1 path outside what was declared, in 1 of 1 session that declared one",
  "                                   outside  turns  no edits   cost",
  "  add rate limiting to /orders",
  "    11111111  01-15 14:02  merged        1      7         2  $3.69",
  "",
  "  primed · no sessions",
  "",
  "  captured · no sessions",
