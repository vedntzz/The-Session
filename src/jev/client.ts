import type { SuggestScopeInput } from "./interface.js";

export async function requestJev(intent: SuggestScopeInput["intent"], paths: SuggestScopeInput["candidatePaths"], timeoutMs = 1000): Promise<unknown | null> {
  ...
  try {
    ...
    return await Promise.race([deadline, fetch(endpoint, {
      ...
      body: JSON.stringify({ intent, paths }),
