...
export function formatAgreement(
  ...
  lines.push(...note("Policy is checked only in a repository where session hook install --enforce has run, for Edit, Write, MultiEdit and shell commands; a shell command the check cannot read is asked about.", palette.meta, width));
