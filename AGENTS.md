...
5. **Cross-tool.** Nothing may assume Codex specifically. Adapters go behind an interface; the core reads a normalised shape.
...
src/  cli.ts registration   commands/ start stop show week scan debt survival pr sweep
      ...
      capture/ hook.ts, adapters/Codex.ts, transcript.ts
...
Rules for one area each, loaded when that area is what you are changing: `.Codex/skills/measurement-rules` (outcome, class, intent source, scan, debt, survival, estimate, money), `.Codex/skills/sync-and-chain` (the line on disk, verify, refs), `.Codex/skills/terminal-output` (CLI surface, colour, Markdown, the pull request body). Why any of it is this way: [docs/decisions.md](docs/decisions.md).
